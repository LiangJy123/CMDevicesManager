...
namespace CMDevicesManager.Services
{
    public sealed class RealSystemMetricsService : ISystemMetricsService
    {
        ...
        private ISensor? _diskRead;
        private ISensor? _diskWrite;

        private bool _loggedCpuTemp, _loggedGpuTemp, _loggedCpuPower, _loggedGpuPower, _loggedCpuLoad, _loggedGpuLoad, _loggedMemLoad;
        private bool _loggedCpuName, _loggedGpuName, _loggedMemName, _loggedDiskRead, _loggedDiskWrite;
        ...
        public RealSystemMetricsService()
        {
            ...
            Logger.Info("[HW] This service reads CPU, GPU, memory, and disk metrics for dashboard display only");
            ...
            try
            {
                _computer = new Computer
                {
                    ...
                    IsStorageEnabled = true,      // Enable storage monitoring for disk I/O
        ...
        public double GetDiskReadKBs() => ReadOrZero(ref _diskRead, () =>
            FindStorageSensor(SensorType.Throughput, s => s.Name.Contains("Read", StringComparison.OrdinalIgnoreCase))) / 1024.0; // Convert to KB/s

        public double GetDiskWriteKBs() => ReadOrZero(ref _diskWrite, () =>
            FindStorageSensor(SensorType.Throughput, s => s.Name.Contains("Write", StringComparison.OrdinalIgnoreCase))) / 1024.0; // Convert to KB/s
        ...
        private void CacheSensors()
        {
            ...
            // Cache disk sensors
            _diskRead = FindStorageSensor(SensorType.Throughput, s => s.Name.Contains("Read", StringComparison.OrdinalIgnoreCase)) ??
                        FindStorageSensor(SensorType.Throughput, _ => true);

            _diskWrite = FindStorageSensor(SensorType.Throughput, s => s.Name.Contains("Write", StringComparison.OrdinalIgnoreCase)) ??
                         FindStorageSensor(SensorType.Throughput, _ => true);
        }
        ...
        private ISensor? FindStorageSensor(SensorType type, Func<ISensor, bool> predicate) =>
            _computer.Hardware
        ...
        private void LogMissing(ISensor? sensor, Exception? ex = null)
        {
            ...
            else if (sensor == _diskRead && !_loggedDiskRead)
            {
                msg = "[HW] Disk read sensor not found. Using 0.";
                Logger.Info(msg + (ex != null ? " " + ex.Message : string.Empty));
                Logger.Info($"[HW] Looking for disk read sensor with pattern: Read");
                _loggedDiskRead = true;
            }
            else if (sensor == _diskWrite && !_loggedDiskWrite)
            {
                msg = "[HW] Disk write sensor not found. Using 0.";
                Logger.Info(msg + (ex != null ? " " + ex.Message : string.Empty));
                Logger.Info($"[HW] Looking for disk write sensor with pattern: Write");
                _loggedDiskWrite = true;
            }
        }
